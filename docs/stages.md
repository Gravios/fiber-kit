...
| `--no-emit-hierarchy` (`--emit-hierarchy`) | True | emit the .clu/.clc/.clp microfiber triple (atoms = pre-link fine fragments, fibers = linked global ids) via FiberHierarchy, instead of a flat .clu only. --no-emit-hierarchy writes just the flat .clu (legacy). Ignored with --out. |
...
| `--no-subsample` (`--subsample`) | — | enable (--subsample) or disable (--no-subsample) realign's per-spike sub-sample (parabolic) refine in the feature build; default leaves the FIBER_SUBSAMPLE env var / lever untouched (off).  Reaches pool workers. |
| `--out` | — |  |
...
| `--no-emit-hierarchy` (`--emit-hierarchy`) | True | after writing the .clu, also emit the .clc/.clp microfiber triple via the fiber-microfiberize identity lift (each fiber = one atom) so the refine output is a hierarchy intrachunk/link can consume; --no-emit-hierarchy writes the flat .clu only |
...
| `--no-ab-reclaim` (`--ab-reclaim`) | False | after merge_back, run the targeted A/B contamination reclaim: move a host cluster's spikes into a clean, DISTINCT donor when their realigned shape matches the donor (refractory-safe, per-spike, reclaim-only).  Recovers foreign spikes a non-co-firing contaminant leaves invisible to the refractory metric.  Default off. |
...
| `--gpu` | flag (off) |  |
| `--feature-align` | — | feature-building alignment: xcorr (default) or centroid (pure, no refine -- adds the trough-position-vs-asymmetry structure to the clustering/linking features).  Does NOT touch committing alignment or fiber-realign.  Overrides the FIBER_ALIGN env var. — choices: `xcorr`, `centroid` |
| `--no-dedup-strict` (`--dedup-strict`) | True | when a dedup removes spikes, every live per-spike file of the group is subset too. A live file at a third row count (neither pre- nor post-dedup) is a stale leftover from an earlier extraction that CANNOT be subset by this mask; by default it is quarantined aside (.stalebkp) and regenerated. --no-dedup-strict leaves such files in place instead; --dedup-stale error restores a hard failure. |
...
| `--variant` | — | staged .clu method (e.g. stderiv) instead of --in-clu |
| `--stage` | — | staged .clu tag (e.g. refine); pair with --variant |
...
| `--n-grid` | `40` |  |
| `--method` | `refine` | method tag in the .fibers filename |
| `--out` | — |  |
...
| `--ground-truth` (`--gt`) | — | ground-truth .clu file |
...
| flag | default | description |
|---|---|---|
| `--output` (`-o`) | `fiber-kit.yaml` | destination path (default: ./fiber-kit.yaml) |
| `--force` (`-f`) | flag (off) | overwrite the destination if it already exists |
